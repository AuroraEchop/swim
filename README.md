...
前端当前已具备登录页、后台主布局、路由守卫、API 客户端、首页统计骨架、船舶管理页面和模块占位页。页面风格以 `DESIGN.md` 为准：浅色后台管理界面，深海青作为主色，整体保持正式、清晰、适合学校课程设计演示。
...
继续按 `前端页面设计文档.md` 和 `DESIGN.md` 实现业务页面：复用船舶管理页的列表、筛选、抽屉表单和状态标签模式，下一步接入船员管理页面。
