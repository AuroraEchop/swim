...
export interface CrewMemberQuery {
  keyword?: string
  position?: string
  shipId?: number
  status?: CrewStatus | ''
...
export function updateCrewMember(id: number, data: UpdateCrewMemberRequest) {
  return request.put<unknown, void>(`/crew-members/${id}`, data)
}

export function updateCrewMemberStatus(id: number, status: CrewStatus) {
  return request.patch<unknown, void>(`/crew-members/${id}/status`, { status })
}

export function deleteCrewMember(id: number) {
  return request.delete<unknown, void>(`/crew-members/${id}`)
}
