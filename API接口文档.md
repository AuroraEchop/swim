...
说明：本课程设计中的 `loginToken` 只作为前后端分离调用时的简单登录标识，可由后端在登录成功后生成普通字符串返回，也可以在课程演示中简化为固定格式字符串。它不要求使用 JWT，不要求实现刷新 Token、复杂会话管理或生产级鉴权机制。若后续实现时间紧张，也可以只在前端保存登录用户信息，后端重点完成用户名和密码校验。
...
接口说明：该接口用于前端刷新页面后获取当前用户信息。课程设计中可通过请求头中的简单 `loginToken` 查询或解析当前用户，也可根据实现情况简化处理。
