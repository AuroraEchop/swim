...
export interface ShipQuery extends PageQuery {
  keyword?: string
  type?: string
  homePort?: string
  status?: ShipStatus
...
export function updateShip(id: number, data: UpdateShipRequest) {
  return request.put<unknown, null>(`/ships/${id}`, data)
}

export function updateShipStatus(id: number, status: ShipStatus) {
  return request.patch<unknown, null>(`/ships/${id}/status`, { status })
}

export function deleteShip(id: number) {
  return request.delete<unknown, null>(`/ships/${id}`)
}
